...
delta_r_1 = 21.5 * math.pi
delta_r_2 = 21.75 * math.pi
...
measurement1 = rgb_method.OneLocation(
    measured_retardations=[
        rgb_method.MeasuredDelta(wavelength=l_r, delta=delta_r_1_tilde),
        rgb_method.MeasuredDelta(wavelength=l_g, delta=delta_g_1_tilde),
        rgb_method.MeasuredDelta(wavelength=l_b, delta=delta_b_1_tilde)],
    reduced_birefringence_function=k_function)

measurement2 = rgb_method.OneLocation(
    measured_retardations=[
        rgb_method.MeasuredDelta(wavelength=l_r, delta=delta_r_2_tilde),
        rgb_method.MeasuredDelta(wavelength=l_g, delta=delta_g_2_tilde),
        rgb_method.MeasuredDelta(wavelength=l_b, delta=delta_b_2_tilde)],
    reduced_birefringence_function=k_function)
...
def L(deltas: list[float, float]):
    return multi_locations.collective_error_function_L(delta_rs=deltas, k=1)




deltas_found = multi_locations.find_all_neighboring_delta_r(k=1,
                                                            lb_delta=0,
                                                            ub_delta=25*math.pi,
                                                            strategy="rand1exp")
