...
def find_parameters_1(measurement: OptimizationProcedure) -> (float, float, float):
...
def find_parameters_2(measurement: OptimizationProcedure) -> (float, float, float):
    parameters = measurement.find_characteristic_parameters(
        lb_delta=0, ub_delta=math.pi,
...
def find_parameters_3(measurement: OptimizationProcedure) -> (float, float, float):
    parameters = measurement.find_characteristic_parameters(
        lb_delta=0, ub_delta=math.pi,
        lb_theta=0, ub_theta=math.pi / 2,
        lb_omega=0, ub_omega=math.pi,
